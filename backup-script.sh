...
install_dependencies() {
    ...
    # Esperar um pouco para os containers estarem prontos
    sleep 10
    
    docker-compose exec app composer install
    
    # Se necessário, use --no-scripts flag se encontrar problemas
...
setup_app_structure() {
    ...
    # Atualizar migration para users - esta já deve existir no framework
...
EOL
    ...
    # Criar migration para travel_requests com nome fixo para evitar problemas de localização do arquivo
    log_info "Criando migration para travel_requests..."
    
    # Criar diretório de migrations se não existir
    mkdir -p database/migrations
    
    # Nome fixo para o arquivo de migração
    TRAVEL_MIGRATION="database/migrations/2025_04_04_000000_create_travel_requests_table.php"
    
    log_info "Criando migração em: $TRAVEL_MIGRATION"
    
    # Criar o arquivo de migração com conteúdo definido
    cat > "$TRAVEL_MIGRATION" << 'EOL'
<?php
...
EOL
    ...
    log_success "Migrations e models configurados com sucesso."
}

# Configurar controllers, requests e resources
setup_controllers_and_resources() {
    log_info "Configurando controllers, requests e resources..."
...
EOL
    ...
    # Configurar JWT no arquivo de configuração
    mkdir -p config/jwt
    cat > config/jwt.php << 'EOL'
<?php
...
EOL
    ...
    # Configurar autenticação para usar JWT
    cat > config/auth.php << 'EOL'
<?php
...
EOL
    ...
    log_success "Controllers, requests e resources configurados com sucesso."
}
...
finalize_setup() {
    ...
    docker-compose exec app php artisan tinker --execute="App\Models\User::create(['name' => 'Admin', 'email' => 'admin@example.com', 'password' => Hash::make('password'), 'role' => 'admin']);"
...
main() {
    ...
    echo "========================================="
    echo ""
    echo "O projeto está disponível em: http://localhost:8000/api"
    echo ""
    echo "Credenciais de administrador para testes:"
